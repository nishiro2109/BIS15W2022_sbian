...
R is an open source, scripting language. RStudio is a graphical user interface (GUI) and an easier way to interact with R. We must installed R or RStudio cannot work by itself. GitHub is useful because it is an easier way to share code with other programmers and by GitHub Desktop we can change our code effectively.
...
## [1] 2
```
  Oh so for the second one it is 8 / (2 ** 2)
