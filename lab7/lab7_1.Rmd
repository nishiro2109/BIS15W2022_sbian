...
#pick an variable in conservation
```

Here we summarize the number of NA's over all of the variables.
```{r}
msleep %>% 
  summarise_all(~(sum(is.na(.)))) #all NAs in data frame
```
...
```{r}
msleep %>%  
  mutate(conservation_modified = na_if(conservation, "domesticated"))%>% # create a new column
