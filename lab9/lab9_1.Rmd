...
+ `discrete` quantitative data that only contains integers //2,3,4
+ `continuous` quantitative data that can take any numerical value //5.1, 5.2
+ `categorical` qualitative data that can take on a limited number of values //drug a,b,c,d
...
```{r}
ggplot(data=homerange, mapping = aes(x=log10.hra, y=log10.preymass)) + geom_point() + geom_smooth(method = lm, se=FALSE)
```
